  ...
  return (
    <div>
      ...
      <ul className="space-y-1">
        {links.map((link) => (
          <li key={link.href}>
            <a href={link.href} className="inline-flex min-h-10 items-center text-sm text-white/50 transition-colors hover:text-brand-glow">
              {link.label}
            </a>
          </li>
        ))}
        ...
        <div className="mb-12 grid grid-cols-2 gap-10 md:grid-cols-4">
          ...
          <div className="col-span-2 md:col-span-1">
            ...
            <ul className="space-y-1 text-sm">
              <li>
                <a href={CONTACT.phoneHref} className="flex min-h-10 items-center gap-2 text-white/50 hover:text-brand-glow">
                  <Phone className="h-4 w-4" /> {CONTACT.phone}
                </a>
              </li>
              <li>
                <a href={`mailto:${CONTACT.email}`} className="flex min-h-10 items-center gap-2 break-all text-white/50 hover:text-brand-glow">
              ...
              {SOCIAL_LINKS.map((link) => {
                ...
                return (
                  <a
                    ...
                    className="flex h-11 w-11 items-center justify-center rounded-full border border-white/15 text-white/60 transition hover:border-brand hover:text-brand-glow"
